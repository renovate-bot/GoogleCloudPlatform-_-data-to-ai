...
#### 1) Combining different vector searches:

You can search and combine results from different models and vector spaces, in our case the text embeddings and multimodal embeddings.
This is a common scenario when combining semantic search results (using dense embeddings) with keyword-based search results (using sparse embeddings).

The Reciprocal Rank Fusion (RRF) algorithm combineds results from different ranking methods into a single, unified ranking. It works by giving higher scores to documents that appear near the top of multiple result sets.

Here's a simplified SQL implementation of RRF, combining results from `semantic_search_multimodal_embeddings`
and `semantic_search_text_embeddings` result sets using the query "a bus stop with broken glass":
...
#### 2) Enhancing vector search with keyword search
...
One approach to combining a keyword search results with semantic search is to boost the results that match the keywords while still primarily ranking based on semantic similarity. This might be useful when you want to boost an exact keyword match like "broken glass" compared to a potential semantic-only match like "broken cup".
...
SELECT 
    ...
    CASE WHEN k.uri IS NOT NULL THEN 1 ELSE 0 END AS keyword_match,  -- Binary keyword match feature
    (1.0 / r.rank) + (CASE WHEN k.uri IS NOT NULL THEN boost_factor ELSE 0 END) AS boosted_score,  -- Boost score for keyword matches
...
You can tune the `boost_factor` variable to control the influence of the Boolean match.
