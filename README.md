...
You can search and combine results from different models and vector spaces, in our case the text embeddings and multimodal embeddings. That said, this approach is typically used when combining semantic search results (using dense embeddings) with keyword-based search results (using sparse embeddings generated by a vectorizer like [TF-IDF](https://en.wikipedia.org/wiki/Tf%E2%80%93idf) or [BM25](https://en.wikipedia.org/wiki/Okapi_BM25)).

For our purposes, we'll just combined the search results from the two deployed embedding models used `text_embedding_models` and `multimodal_embedding_models`, but the same approach applies with other pre-trained or custom embedding models.

To that end, the [Reciprocal Rank Fusion (RRF)](https://cloud.google.com/vertex-ai/docs/vector-search/about-hybrid-search#rrf) algorithm is commonly used to combine results from different ranking methods into a single, unified ranking. It works by giving higher scores to documents that appear near the top of multiple result sets.

Here's a simplified SQL implementation of RRF, combining results from `semantic_search_multimodal_embeddings`
and `semantic_search_text_embeddings` result sets from the query "a bus stop with broken glass":
